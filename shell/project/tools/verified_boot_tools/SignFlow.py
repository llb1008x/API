...
def setPath():
	...
	global img_ver_path
	global bin_tmp_path
	out_path = raw_img_path
	bin_tmp_path = os.path.join(out_path, "resign/bin/multi_tmp/")
	resign_tool_path = "gionee/config/tools/verified_boot_tools/resignTool.py"
	cert1_dir = "gionee/config/tools/verified_boot_tools/configs/cert1/"
	cert2_key_dir = "gionee/config/tools/verified_boot_tools/configs/cert2_key/"
	img_list_path = "gionee/config/tools/verified_boot_tools/configs/img_list.txt"
	img_ver_path  = "gionee/config/tools/verified_boot_tools/configs/img_ver.txt"
...
def main():
	...
	for bin in singleBinDict.keys():
		...
		img_path = os.path.join(out_path, bin)
	...
	for bin in multiBinDict.keys():
		...
		multi_tmp_bin_out = os.path.join(out_path, "resign/bin/"+ bin.split(".")[0]+"-verified."+bin.split(".")[1])
		img_path = os.path.join(out_path, bin)
		copyFile(img_path, multi_tmp_bin_in)
		...
		for img_name in img_name_list:
			...
			if (img_name == "atf"):
				copyFile("release/tmp/resign/cert/cert2/atf/intermediate/tmp_bin/trustzone-verified.bin",multi_tmp_bin_in)
			elif (img_name == "tee"):
				copyFile("release/tmp/resign/cert/cert2/atf/intermediate/tmp_bin/trustzone-verified.bin",multi_tmp_bin_in)
			print "img_name = {} copyFile 22222222222".format(img_name)
