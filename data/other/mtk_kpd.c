...
{ 
    ...
    for (i = 0; i < KPD_NUM_MEMS; i++) {  
        ...
        if (!change)  //按键状态没有变化的直接进入下一次循环
            continue;  
    
        for (j = 0; j < 16; j++) {  //这应该是把变化的状态为取出来
            mask = 1U << j;  
            if (!(change & mask))  
                continue;  
...
程中是不会被其他任务打断的。
4.所以说这里的操作是当有按键被按下的时候pressed=1，设置KEY_VOLUMEUP设置为1，KEY_VOLUMEDOWN设置为0，其他直接返回
当没有被按下的时候将相应的寄存器位直接清除，然后把状态上报
