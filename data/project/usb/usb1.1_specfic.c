...
1.1  起因
    ...
    (一)计算机与电话之间的连接：显然用计算机来进行计算机通信将是下一代计算机基本的应用。机器和人们的数据交互流动需要一个广泛而又便宜的连通网络。
    然而，由于目前产业间的相互独立发展,尚未建立统一标准,而USB则可以广泛的连接计算机和电话。
    (二)易用性：众所周知,PC机的改装是极不灵活的。对用户友好的图形化接口和一些软硬件机制的结合，加上新一代总线结构使得计算机的冲突大量减少，且易于改装。
    但以终端用户的眼光来看，PC机的输入/输出，如串行/并行端口、键盘、鼠标、操纵杆接口等，均还没有达到即插即用的特性，USB正是在这种情况下问世的。
    (三)端口扩充：外围设备的添加总是被相当有限的端口数目限制着。缺少一个双向、价廉、与外设连接的中低速的总线，限制了外围设备(诸如电话/电传/调制解调器
        的适配器、扫描仪、键盘、PDA)的开发。现有的连接只可对极少设备进行优化，对于PC机的新的功能部件的添加需定义一个新的接口来满足上述需要，USB就应
        运而生。它是快速、双向、同步、动态连接且价格低廉的串行接口，可以满足PC机发展的现在和未来的需要。
 
1.2  USB规范的目标
    本书规范了USB的工业标准。该规范介绍了USB的总线特点、协议内容、事务种类、总线管理、接口编程的设计，以及建立系统、制造外围设备所需的标准。
    设计USB的目标就是使不同厂家所生产的设备可以在一个开放的体系下广泛的使用。该规范改进了便携商务或家用电脑的现有体系结构，进而为系统生产商和外设开发
    商提供了足够的空间来创造多功能的产品和开发广阔的市场，并不必使用陈旧的接口，害怕失去兼容性。
...
1.4  本书结构
    ...
    ·USB 设备驱动厂家主要参考第四、七、九章
     《Universal Serial Bus Device Class Specification》一书可以作为本书的补充和参考。各种设备的规范是形形色色的，如有疑问，请与
     USB Implements Forum索要更多细节。
...
2.2  使用的分类
表2-1按照数据传输率(USB可以达到)进行了分类。可以看到，12M比特率可以包括中速和低速的情况。总的来说，中速的传输是同步的，低速的数据来自交互的设备，
USB设计的初衷是针对桌面电脑而不是应用于可移动的环境下的。软件体系通过对各种主机控制器提供支持以保证将来对USB的扩充。
