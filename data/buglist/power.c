...
基本的概念：	
{
	...
	Mobile radio active
	　之所以“Mobile radio active”如此耗电，是因为该功能控制了应用访问数据流量的权限，而系统将决定手机在锁屏后应用是否能够继续联网，
	在出现这个Bug的情况下，手机在熄屏很久以后可能都无法进入睡眠状态。
...
-->	分析功耗一些相关的文件导出步骤.
	...
	功耗测试前，可通过adb 命令：adb shell am start com.gionee.cloud.gpe/.ui.PushActivity 打开的界面查看push推送开关是否都关闭了
...
-->
	...
	modem常用的几个信道，哪些跟耗电密切相关的？
	{
		...
		*channel 20代表网络连接需要结合main.log，这个比较多通常是apk数据交互频繁
		20都是数据业务的连接，影响会比较大
		是网络发起的还是应用发起的？
		->是APK主动发包，20是网络返回数据给APK. 造成的额外功耗，主要因素是APK发包.
		...
		EINT 206 是QQ/微信/金立云/phone的apk唤醒.
		CLDMA_WDT (10)是反馈数据连接的状态.基本上出现CLDMA_WDT (20)就会有10.
		CLDMA_WDT (20)是代表有数据连接发送到手机端.
		CONN2AP是wifi的唤醒.
		EINT212就是跟C2K的唤醒
	...
	金立推送( com.gionee.cloud.gpe)产生耗电的原因:
    	心跳机制，在网络正常的情况下且有推送开关处于开启状态，金立推送为保持和服务器的长连接默认会6分钟发送一次心跳，受Alarm对齐后会10分钟发送一次心跳。
    每次都会申请wakelock，每次持有时间大概10-20毫秒，这样肯定会产生耗电的。
	这个推送主要涉及金立相关的应用和系统服务上,而且这个推送直到后半夜才会停止push。

		网络没有配置drx（网络决定，手机默认就有），所以modem 发数据的电流会很大
		可以follow一下每个TCP 的会话，你会发现，基本上都是长连接，这部分应该还是贵司自己的应用.另外电流图看，网络没有配置DRX，
只要有一个连接，就会导致系统电流持续时间比较长.APK发包的影响占比最大的，只要有发包，整体的电流就会很大
这个网络配置跟特定的运营商有关会有差别的，移动卡跟电信跟联通表现都不一样，主要表现是在持续的时间上面.
...
信这样做就很有问题）
	...
	通过adb 命令：adb shell am start com.gionee.cloud.gpe/.ui.PushActivity 打开的界面查看push推送开关是否都关闭了
	{
		...
		这个单位应该是毫秒把
		--------> 是秒，大概有14h.
		金立推送这个建立的是一个长连接，可能持续从你联网到最后断网才会停止，因为服务器要时刻清楚客户端的状况，而且还有同步心跳唤醒
		上报客户端的状态。每10min是定期的唤醒，但是长连接一直存在，所以push耗电还是很高

		2.之前分析的时候看systemlog，基本上是每10min一次唤醒，mainlog里面除了gionee.cloud.gpe还有很多其他的，比如
		05-23 17:22:28.184176 5479 6494 D Posix : [Posix_connect Debug]Process com.android.browser :14000
		浏览器有做什么操作吗？
		-------->这部分是短连接，影响会比较小，25次的话，如果以6min一次，也就是2.5h. 影响没有push这么大.

		3.这种插双卡待机的有什么要注意的地方吗？
		------->联通卡的电流会很大，paging会比较密集，另外联通的卡，modem的开销会比较大，没有drx模式，一个电流起来是持续10~20s高电平
		在没有唤醒的情况下，2张联通卡大概是20mA，而移动卡大概只有10mA，所以联通卡的待机电流都会比移动的大
		因为联通基站比较少，而且基站没有相应的配置，所以联通卡待机功耗很高
	}



	push是否会停止运行和时间没有关系。
	以下情况push会停止运行，停止发包，停止定期唤醒。
	1）没有网络连接。
	2）push推送开关都关闭。
	总是感觉有问题，后半夜还是会有网络链接，但是gionee.cloud却是没有了


	1602上出现的锁屏壁纸更新引起的电流变高，功耗高：
		屏更新在待机过程中如果没有更新，可能在待机过程中请求更新，有联网操作，出现异常,而锁屏壁纸通常是12时更新的
	

    wifi如果开的话正常在十几个小时内都有150mA左右的电流，wifi on和wifi running
    而网络信号不好的话通常signal level很差，如果kernel里面CLMDA 10比较多这是因为信号经常变化
    手机找信号，这时radio types里面通常不好

    cn.richinfo.dm
    移动DM这个主要是修改服务端的配置，上报频率和心跳频率都是服务端配置，由于目前DM还在调试阶段，经常改成调试配置，导致测试的结果混乱，
    前天商定后期的软件版本直接核入商用的DM，频率为24小时一次
	...
	-------》
	我看了掉电比较快的部分是12点以前. 这段时间掉电的时间比较短，从netlog看的话，这部分的tcp交互比较频繁.
	另外如果有通讯功耗会比较大，有一个ack的动作，电流就会起来，跟数据量关系不是很大.

	一般情况下，电信卡跟联通卡这部分的功耗会比较差，移动会比较好.

	你可以实际测试一下电流double check一下平均电流.


	#77154
	跟GPS的owner沟通，之前也有遇到类似的问题，现在有些基站会要求定位，会无条件打开手机的GPS来定位.
导致功耗偏高，这部分的话属于网络的异常.
