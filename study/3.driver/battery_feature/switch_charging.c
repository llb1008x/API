...
kal_bool is_enter_mmi_test = KAL_FALSE;
//#endif
...
/* ============================================================ // */
 ...
 extern kal_bool upmu_is_chr_det(void);	//Gionee GuoJianqiu 20150603 modify for fix CR01492674
...
*/
	...
	//GioneeDrv GuoJianqiu 20151223 modify for platform change end
		...
		/* set IOCHARGE */
		if (g_bcct_value < 550)
			g_temp_CC_value = CHARGE_CURRENT_500_00_MA;	//Gionee GuoJianqiu 20150610 modify for fix CR01498761
			...
			g_temp_CC_value = CHARGE_CURRENT_1500_00_MA;	//Gionee GuoJianqiu 20150417 modify for fix errs begin
		/* --------------------------------------------------- */
	...
	//Gionee GuoJianqiu 20150608 modify for fix CR01498099 begin
	battery_log(BAT_LOG_CRTI,
				    "[BATTERY] select_charging_curret_bcct : %d, input current = %d\r\n",
				    g_temp_CC_value, g_temp_input_CC_value);
	//Gionee GuoJianqiu 20150608 modify for fix CR01498099 end
...
unsigned int set_bat_charging_current_limit(int current_limit)
{
	...
	/* wake_up_bat(); */
	if (upmu_is_chr_det() == KAL_TRUE)	//Gionee GuoJianqiu 20150603 modify for fix CR01492674
		pchr_turn_on_charging();
...
static unsigned int charging_full_check(void)
{
	...
	//GioneeDrv GuoJianqiu 20151223 modify for platform change begin
	if ( status == KAL_TRUE && BMT_status.bat_vol >= RECHARGING_VOLTAGE) {
		g_full_check_count++;
		...
		//Gionee GuoJianqiu 20141130 modify for GNSPR00027029 begin
		return KAL_FALSE;
		//Gionee GuoJianqiu 20141130 modify for GNSPR00027029 end
	}
	//GioneeDrv GuoJianqiu 20151223 modify for platform change end
}
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
    ...
    //Gionee GuoJianqiu 20141130 modify for GNSPR00027029
	if (charging_full_check() == KAL_TRUE && BMT_status.bat_vol >= RECHARGING_VOLTAGE) {
		BMT_status.bat_charging_state = CHR_BATFULL;
		BMT_status.bat_full = KAL_TRUE;
		g_charging_full_reset_bat_meter = KAL_TRUE;
	}
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	//Gionee GuoJianqiu 20141130 modify for GNSPR00027029
	if (charging_full_check() == KAL_FALSE || BMT_status.bat_vol < RECHARGING_VOLTAGE) {
...
void mt_battery_charging_algorithm(void)
{
	...
	if( gn_switch_charging_state==KAL_TRUE ){
			...
			battery_log(BAT_LOG_CRTI, "22222222222222222222222222222222222222222\n");
			if( (upmu_is_chr_det() == KAL_TRUE)&&(BMT_status.charger_type==STANDARD_CHARGER) ){

				battery_log(BAT_LOG_CRTI, "333333333333333333333333333333333333333\n");
				if( BMT_status.UI_SOC2 < gn_switch_charging_down ){

						battery_log(BAT_LOG_CRTI, "4444444444444444444444444444444444444444\n");
	...
	battery_log(BAT_LOG_CRTI, "gn_switch_charging_down->%d,gn_switch_charging_up->%d\n",
