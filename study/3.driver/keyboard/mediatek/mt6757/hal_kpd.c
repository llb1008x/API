...
void kpd_slide_qwerty_init(void)
{
	...
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad begin
	kpd_info("arrive at powerOn_slidePin_interface\n");
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad end
	...
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad begin
	kpd_info("arrive at powerOff_slidePin_interface\n");
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad end
...
//Gionee <gn_by_charging> <lilubao> <20170406> add for change reboot begin	
	int ret=0;
	kpd_info("in hal_kpd.c long_press_reboot_function_init_pmic before by lilubao\n");		

	kpd_info("[PMIC] Reg[0x%x]=0x%x before by lilubao\n",
	...
	kpd_info("Enable %s mode LPRST\n", mode);
		...
		ret=pmic_set_register_value(KPD_PMIC_RG_PWRKEY_RST_EN, 0x01);
		kpd_info("[PMIC] Reg[0x%x]=0x%x after by lilubao\n",
			KPD_PMIC_RG_PWRKEY_RST_EN, upmu_get_reg_value(KPD_PMIC_RG_PWRKEY_RST_EN)); 
		kpd_info("1ret->%d\n",ret);
		
		ret=pmic_set_register_value(KPD_PMIC_RG_HOMEKEY_RST_EN, 0x00);
		kpd_info("2ret->%d\n",ret);
		kpd_info("[PMIC] Reg[0x%x]=0x%x after by lilubao\n",
			KPD_PMIC_RG_HOMEKEY_RST_EN, upmu_get_reg_value(KPD_PMIC_RG_HOMEKEY_RST_EN)); 
		
		ret=pmic_set_register_value(KPD_PMIC_RG_PWRKEY_RST_TD, CONFIG_KPD_PMIC_LPRST_TD);
		kpd_info("3ret->%d\n",ret);
		kpd_info("[PMIC] Reg[0x%x]=0x%x after by lilubao\n",
			KPD_PMIC_RG_PWRKEY_RST_TD, upmu_get_reg_value(KPD_PMIC_RG_PWRKEY_RST_TD)); 
...
#endif
	kpd_info("[PMIC] Reg[0x%x]=0x%x between by lilubao\n",
			MT6355_TOP_RST_MISC, upmu_get_reg_value(MT6355_TOP_RST_MISC)); 
...
{
	//Gionee <gn_by_charging> <lilubao> <20170406> add for change reboot begin	
	kpd_info("in hal_kpd.c kpd_pmic_rstkey_hal before by lilubao\n");
	kpd_info("kpd_dts_data.kpd_sw_rstkey->%d,kpd_sb_enable->%d\n",kpd_dts_data.kpd_sw_rstkey,kpd_sb_enable);	
		...
		if (!kpd_sb_enable) {
			...
			if (kpd_show_hw_keycode) {
				kpd_print(KPD_SAY "(%s) HW keycode =%d using PMIC\n",
					...
					kpd_info("1111111111111111111111\n");
			}
			kpd_info("22222222222222222222222\n");
		}
		kpd_info("3333333333333333333\n");
	}
	kpd_info("in hal_kpd.c kpd_pmic_rstkey_hal end by lilubao\n");
	//Gionee <gn_by_charging> <lilubao> <20170406> add for change reboot end	
}
...
{
	//Gionee <gn_by_charging> <lilubao> <20170406> add for change reboot begin	
	kpd_info("in hal_kpd.c kpd_pmic_pwrkey_hal before by lilubao\n");
	kpd_info("kpd_dts_data.kpd_sw_rstkey->%d,kpd_sb_enable->%d\n",kpd_dts_data.kpd_sw_rstkey,kpd_sb_enable);	
	...
	if (!kpd_sb_enable) {
		...
		if (kpd_show_hw_keycode) {
			...
			kpd_info("44444444444444444444\n");
		}
		aee_powerkey_notify_press(pressed);
		kpd_info("5555555555555555555555555\n");
	}
#endif
	kpd_info("in hal_kpd.c kpd_pmic_pwrkey_hal end by lilubao\n");
	//Gionee <gn_by_charging> <lilubao> <20170406> add for change reboot end
}
