...
struct battery_data {
	...
	int BAT_BatteryPresentCurrent; 	
	//Gionee GuoJianqiu 20150318 modify for fix bugs end
...
unsigned int g_gn_screenon_time = 0;
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		...
		break;
	//Gionee GuoJianqiu 20150424 modify for fix bugs begin
	case POWER_SUPPLY_PROP_BatteryPresentCurrent: 	
...
static ssize_t store_Otg_Charge_Switch(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	rv = kstrtouint(buf, 0, &otg_charge_state);
	if (rv != 0)
		{
		battery_log(BAT_LOG_CRTI, "  bad argument, echo [enable] > Otg_Charge_Switch! gn_otg_charge_switch_State = %d\n", gn_otg_charge_switch_State);
		return -EINVAL;
		}
	if(otg_charge_state)	otg_charge_state = KAL_TRUE;
	
	if(otg_charge_state != gn_otg_charge_switch_State)
	{
		msleep(150);
		...
		battery_log(BAT_LOG_CRTI, "store_otg_charge_switch_State == otg_charge_state, do nothing!! \n");
...
int gn_switch_charging_state=KAL_FALSE;

//if you have open change this state ,you could setting charging range
static ssize_t show_Switch_Charging_State(struct device *dev,struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "show gn_switch_charging_state = %d\n", gn_switch_charging_state);    
    return sprintf(buf, "%u\n", gn_switch_charging_state);
}

static ssize_t store_Switch_Charging_State(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	int rv;
...
static DEVICE_ATTR(Switch_Charging_State, 0664, show_Switch_Charging_State, store_Switch_Charging_State);


//setting the least value if you want custom you charging range 
unsigned int gn_switch_charging_down=60;
static ssize_t show_Switch_Charging_Down(struct device *dev,struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "show gn_switch_charging_down = %d\n", gn_switch_charging_down);    
    return sprintf(buf, "%u\n", gn_switch_charging_down);
}

static ssize_t store_Switch_Charging_Down(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	int rv;
	int switch_value=0;
	rv = kstrtouint(buf, 0, &switch_value);
	if ( rv != 0 )
	{
		battery_log(BAT_LOG_CRTI, "bad argument, echo [enable] > Switch_Charging_Value! gn_switch_charging_down = %d\n", gn_switch_charging_down);
		return -EINVAL;
	}else if(switch_value<60){

		battery_log(BAT_LOG_CRTI, "bad argument,the gn_switch_charging_down value must greater than 60\n");
		return -EINVAL;
	}
	gn_switch_charging_down=switch_value;
	battery_log(BAT_LOG_CRTI, "store gn_switch_charging_down = %d\n", gn_switch_charging_down); 

	return size;

}

static DEVICE_ATTR(Switch_Charging_Down, 0664, show_Switch_Charging_Down, store_Switch_Charging_Down);


//setting the max value ,if you want custom you charging range
unsigned int gn_switch_charging_up=100;
static ssize_t show_Switch_Charging_Up(struct device *dev,struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "show gn_switch_charging_up = %d\n", gn_switch_charging_up);    
    return sprintf(buf, "%u\n", gn_switch_charging_up);
}

static ssize_t store_Switch_Charging_Up(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	int rv;
	int switch_value=0;
	rv = kstrtouint(buf, 0, &switch_value);
	if (rv != 0)
	{
		battery_log(BAT_LOG_CRTI, "bad argument, echo [enable] > Switch_Charging_Value! gn_switch_charging_up = %d\n", gn_switch_charging_up);
		return -EINVAL;
	}else if( (switch_value>100)||(switch_value<gn_switch_charging_down) ){

		battery_log(BAT_LOG_CRTI, "bad argument,the gn_switch_charging_up value must in the range,switch_value->%d\n",switch_value);
		return -EINVAL;
	}
	gn_switch_charging_up=switch_value;
	battery_log(BAT_LOG_CRTI, "store gn_switch_charging_up = %d\n", gn_switch_charging_up); 

	return size;

}

static DEVICE_ATTR(Switch_Charging_Up, 0664, show_Switch_Charging_Up, store_Switch_Charging_Up);

#endif
//Gionee <gn_by_charging> <lilubao> <20170416> add for new feature end
...
static ssize_t show_Enter_MMI_Test(struct device *dev,struct device_attribute *attr, char *buf)
{
    is_enter_mmi_test = KAL_TRUE;
	//Gionee GuoJianqiu 201601026 modify for OTG SWITCH begin
	//#if defined(GN_BATTERY_OTGCHARGE_SWITCH_SUPPORT)
	//if(!gn_otg_charge_switch_State)
	//gn_Open_Otg_Irq();
	//#endif
	//Gionee GuoJianqiu 201601026 modify for OTG SWITCH end
    battery_log(BAT_LOG_CRTI, "is_enter_mmi_test = %d\n", is_enter_mmi_test);    
    return sprintf(buf, "%u\n", is_enter_mmi_test);
}
static ssize_t store_Enter_MMI_Test(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	//GioneeDrv GuoJianqiu 20160429 modify for platform change begin
	unsigned int val;

	if (kstrtouint(buf, 10, &val) == 0) {
		is_enter_mmi_test = val;
		battery_log(BAT_LOG_CRTI, "is_enter_mmi_test= %d\n", is_enter_mmi_test);
		return size;
	}
	...
	/* hidden else, for sscanf format error */
	{
		battery_log(BAT_LOG_CRTI, "  bad argument, echo [enable] > Enter_MMI_Test\n");
	}

	return 0;
	//GioneeDrv GuoJianqiu 20160429 modify for platform change end
}
static DEVICE_ATTR(Enter_MMI_Test, 0664, show_Enter_MMI_Test, store_Enter_MMI_Test);

static ssize_t show_Exit_MMI_Test(struct device *dev,struct device_attribute *attr, char *buf)
{
    is_enter_mmi_test = KAL_FALSE;
	//Gionee GuoJianqiu 201601026 modify for OTG SWITCH begin
	//#if defined(GN_BATTERY_OTGCHARGE_SWITCH_SUPPORT)
	//if(!gn_otg_charge_switch_State)
	//gn_Close_Otg_Irq();
...
static ssize_t store_Exit_MMI_Test(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	//GioneeDrv GuoJianqiu 20160429 modify for platform change begin
	unsigned int val;
	...
	/* hidden else, for sscanf format error */
	{
		battery_log(BAT_LOG_CRTI, "  bad argument, echo [enable] > Exit_MMI_Test\n");
	}

	return 0;
	//GioneeDrv GuoJianqiu 20160429 modify for platform change end
}
static DEVICE_ATTR(Exit_MMI_Test, 0664, show_Exit_MMI_Test, store_Exit_MMI_Test);
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	//Gionee GuoJianqiu 20150424 modify for fix bugs begin
	bat_data->BAT_BatteryPresentCurrent=battery_meter_get_charging_current(); 
	//Gionee GuoJianqiu 20150318 modify for fix bugs end
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (bat_data->BAT_CAPACITY <= DLPT_POWER_OFF_THD) {
		...
		if (dlpt_check_power_off() == 1) {
			...
			//Gionee GuoJianqiu 20161113 modify for fix GNSPR #54689 begin
			if (cnt >= 3)
			//Gionee GuoJianqiu 20161113 modify for fix GNSPR #54689 end
...
{
	return (BMT_status.bat_in_recharging_state /*|| BMT_status.bat_full == KAL_TRUE*/);	//Gionee GuoJianqiu 20141226 modify for GNSPR00031054
}
...
void mt_battery_GetBatteryData(void)
{
	...
	//Gionee GuoJianqiu 20150424 modify for fix bugs end
	//Gionee GuoJianqiu 20150611 modify for CR01498036 begin
	if(mt_get_bl_brightness() != 0)
		g_gn_screenon_time = g_gn_screenon_time + 10;
	else
		g_gn_screenon_time = 0;
	battery_log(BAT_LOG_CRTI,"g_gn_screenon_time=(%d), g_call_state=%d, g_boot_reason=%d, g_boot_mode=%d\n", g_gn_screenon_time, g_call_state, get_boot_reason(),get_boot_mode());
	//Gionee GuoJianqiu 20150611 modify for CR01498036 end
}
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
        ...
        //Gionee GuoJianqiu 20150318 modify for fix bugs begin
        else if ( BMT_status.charger_vol < (batt_cust_data.v_charger_max-200) ) 
        {
        	if (BMT_status.charger_protect_status == charger_OVER_VOL)
        	{
			battery_log(BAT_LOG_CRTI, "[BATTERY] charger_vol = %d, recharging!!\n\r", BMT_status.charger_vol);
			BMT_status.bat_charging_state = CHR_PRE;
...
//Gionee GuoJianqiu 20141210 modify for GNSPR00027866 begin
static void gn_update_BatteryPresentCurrent(void)
{
    struct battery_data *bat_data = &battery_main;
    struct power_supply *bat_psy = &bat_data->psy;

	bat_data->BAT_ChargerVoltage = battery_meter_get_charger_voltage();
       bat_data->BAT_BatteryPresentCurrent = battery_meter_get_charging_current();
	battery_log(BAT_LOG_CRTI, "[gn_update_BatteryPresentCurrent] BAT_ChargerVoltage = (%d), PresentCurrent = (%d)\n", bat_data->BAT_ChargerVoltage, bat_data->BAT_BatteryPresentCurrent);
...
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
		...
		battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &CHR_Type_num);
		BMT_status.charger_type = CHR_Type_num;
		//Gionee GuoJianqiu 20151201 modify for fix CR01595060 begin
		if((BMT_status.charger_type == NONSTANDARD_CHARGER) || 
			(BMT_status.charger_type == CHARGING_HOST) ||
			(BMT_status.charger_type == STANDARD_HOST))
		{
			battery_log(BAT_LOG_CRTI, "[mt_charger_type_detection] charger type(%d), detection again!! \r\n",BMT_status.charger_type);
			battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE,&CHR_Type_num);
	    	BMT_status.charger_type = CHR_Type_num;
		}
		//Gionee GuoJianqiu 20151201 modify for fix CR01595060 end
...
static void mt_kpoc_power_off_check(void)
{
	...
	if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
	    || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
		//Gionee GuoJianqiu 201601117 modify for GNSPR #54468 begin
		if ((upmu_is_chr_det() == KAL_FALSE) && (BMT_status.charger_vol < 2500) && (g_bat_init_flag == KAL_TRUE)) {	/* vbus < 2.5V */
		//Gionee GuoJianqiu 201601117 modify for GNSPR #54468 end
			battery_log(BAT_LOG_CRTI,
...
{
	battery_log(BAT_LOG_CRTI, "******** battery : bat_update_thread_wakeup  ********\n"); 	//Gionee GuoJianqiu 20150318 modify for fix bugs
...
void check_battery_exist(void)
{
	...
	if (baton_count >= 3) {
		if ((g_platform_boot_mode == META_BOOT) || (g_platform_boot_mode == ADVMETA_BOOT)
			...
			battery_log(BAT_LOG_CRTI,
					    "[BATTERY] Battery is not exist, power off Charge IC and system (%d)\n",	//Gionee GuoJianqiu 20150318 modify for fix bugs
				    baton_count);

			battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
			//Gionee GuoJianqiu 20150724 modify for GNSPR00037414 begin
			#ifdef CONFIG_MTK_POWER_PATH_MANAGEMENT_SUPPORT
			//battery_charging_control(CHARGING_CMD_SET_PLATFORM_RESET, NULL);
			#else
			//battery_charging_control(CHARGING_CMD_SET_POWER_OFF, NULL);
			#endif
			//Gionee GuoJianqiu 20150724 modify for GNSPR00037414 end
		}
	}
#endif
}
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	//Gionee GuoJianqiu 20150424 modify for fix bugs begin
	unsigned int hv_voltage = (batt_cust_data.v_charger_max + 1000) * 1000;
	//Gionee GuoJianqiu 20150424 modify for fix bugs end
	kal_bool hv_status;
	//Gionee GuoJianqiu 20141210 modify for GNSPR00027866 begin
	static unsigned int gn_update_counter = 0;
	//Gionee GuoJianqiu 20141210 modify for GNSPR00027866 end
...
void charger_hv_detect_sw_workaround_init(void)
{
	...
	if (IS_ERR(charger_hv_detect_thread)) {
		battery_log(BAT_LOG_CRTI,	//Gionee GuoJianqiu 20150318 modify for fix bugs
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		//Gionee GuoJianqiu 20150514 modify for GNNCR00010646 begin
		//#if defined(CONFIG_GN_DUBLE_CHARGE_IC_SUPPORT)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Enter_MMI_Test);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Exit_MMI_Test);
		//#endif
		//Gionee GuoJianqiu 20150514 modify for GNNCR00010646 end
		...
		//Gionee <gn_by_charging> <lilubao> <20170416> add for new feature begin
		#if defined(GN_BATTERY_SWITCH_CHARGING)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Switch_Charging_State);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Switch_Charging_Down);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Switch_Charging_Up);
		#endif
		//Gionee <gn_by_charging> <lilubao> <20170416> add for new feature end
