...
void callback(GtkWidget *widget, gpointer data)
{
	...
	if(TRUE == is_press){
		// 自动图标换成暂停图标
		load_image(image, "./skin/pause.bmp", image->allocation.width, image->allocation.height);
		is_press = FALSE;
	}else{
		// 暂停图标换成自动图标
		load_image(image, "./skin/play.bmp", image->allocation.width, image->allocation.height);
		is_press = TRUE;
	}
}
...
int main(int argc,char **argv)
{
	...
	image = glade_xml_get_widget(gxml, "image");
	button = glade_xml_get_widget(gxml, "button");
	
	//信号连接
	glade_xml_signal_autoconnect(gxml);
	g_signal_connect(button, "clicked", G_CALLBACK(callback), image);	// 按钮回调函数
