...
static int key_sysfs_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
	
	len = sprintf(buf, "Versions: keyboard v1.0\n");
	return len;
}
static int key_sysfs_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t len)
{
	printk(KERN_WARNING "key_sysfs_store:%s\n",buf);
	return sizeof(buf);
}

static DEVICE_ATTR(key_attr, S_IRUGO | S_IWUSR,key_sysfs_show, key_sysfs_store);
...
static int demo_key_probe(struct platform_device *pdev)
{
	...
	input_key->id.vendor = 0xaaaa;
	input_key->id.vendor = 0xbbbb;
	...
	ret = device_create_file(&input_key->dev,&dev_attr_key_attr);
	if(ret)
		goto input_register_device_fail;
...
{
	printk(KERN_WARNING "%s\n",__FUNCTION__);
	device_remove_file(&input_key->dev,&dev_attr_key_attr);
