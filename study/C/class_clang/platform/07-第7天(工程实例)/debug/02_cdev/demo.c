...
static int __init demo_init(void)
{
	...
	printk(KERN_CRIT "zai %s\n",__FUNCTION__);
	//动态申请主从设备号
	alloc_chrdev_region(&dev_num,3,2,"my_cdev");
	//初始化cdev结构体，并且填充fops结构体
	cdev_init(&my_cdev, &f_ops);
	//将cdev结构体与主从设备号进行关联，并且添加到内核
	cdev_add(&my_cdev,dev_num,2);
	
	my_class = class_create(THIS_MODULE, "my_class");
	device_create(my_class,NULL,MKDEV(MAJOR(dev_num),3),NULL,"zcwsy_cdev0");
	device_create(my_class,NULL,MKDEV(MAJOR(dev_num),4),NULL,"zcwsy_cdev1");
...
static void __exit demo_exit(void)
{
	...
	cdev_del(&my_cdev);
	device_destroy(my_class,MKDEV(MAJOR(dev_num),3));
	device_destroy(my_class,MKDEV(MAJOR(dev_num),4));
