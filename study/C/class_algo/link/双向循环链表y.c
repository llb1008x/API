...
{
	STU *pb = head,*pf = head;
	if(head == NULL)//如果头文件为空时，新插入一个...
{
		head = p_new;//把p_new 的值给头文件
		head->next = head;//头文件的地址给了头文件的next
		head->prior = head;//头文件的地址给了头文件的prior
	}
else如果头文件不为空时，就不看上面的if了，
{
		while((p_new->num >= pb->num)&&(pb->next != head)当新指针的数值大于pb的数值
并且pb指向next不是头文件时
{
			pf = pb;
			pb = pb->next;
		}
		if(p_new->num < pb->num)//如果新指针的数值小于pb的数值
{
			if(pb == head)//如果pb的值就是头文件时
{
				pf = head->prior;//头文件的prior就是pf的地址
				pf->next = p_new;//pf的next就是新指针的地址
				p_new->prior = pf;//pf的地址就是新指针prior的地址
				p_new->next = pb;//pb的地址就是新指针next的地址
				pb->prior = p_new;//新地址的地址就是pb的prior的地址
				
				// pb->prior->next=p_new;
				// p_new->prior=pb->prior;
				// p_new->next=pb;
				// pb->prior=p_new;
				head = p_new;
			}else{
			...
			p_new->next = head;
			head->prior = p_new;
			// head->prior->next=p_new;
			// p_new->prior=head->prior;
			// p_new->next=head;
			// head->prior=p_new;
		}
	}
