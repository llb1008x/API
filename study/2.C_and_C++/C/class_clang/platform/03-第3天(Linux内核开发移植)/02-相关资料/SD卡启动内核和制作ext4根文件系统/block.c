...
#define SMDKV210_MMC_SUPPORT_MULTI_PART 1
#if SMDKV210_MMC_SUPPORT_MULTI_PART
typedef struct 
{
	int32_t start_sec;
	int32_t sec_num;
}smdkv210_mmc_part_info;
/*
...
#define SDMMC_KERNEL_SIZE      		(5*SZ_1M)	 /* Kernel 5M */
#define SDMMC_UBOOT_SIZE      		(1*SZ_1M)    /* uboot 1M  */
...
#define SMDKV210_MMC_MAX_PARTITIONS   4
static smdkv210_mmc_part_info smdkv210_mmc_partitions[SMDKV210_MMC_MAX_PARTITIONS];
/* 1 block=512B */
static int smdkv210_init_sdmmc_parition(uint32_t card_size_in_blocks)
{
...
//	smdkv210_mmc_partitions[0].start_sec = 0;
//	smdkv210_mmc_partitions[0].sec_num =0;
...
#endif
	printk(KERN_INFO "edu :112 card_size_in_blocks = %d \n",card_size_in_blocks);


	/* 1M Kernel */
	smdkv210_mmc_partitions[0].start_sec = 0;
	smdkv210_mmc_partitions[0].sec_num =SDMMC_UBOOT_BLK_NUM;	
	/* 5M Kernel */
	smdkv210_mmc_partitions[1].start_sec = SDMMC_UBOOT_BLK_NUM;
	smdkv210_mmc_partitions[1].sec_num =SDMMC_KERNEL_BLK_NUM;
	/* 128M rootfs */
	smdkv210_mmc_partitions[2].start_sec = SDMMC_KERNEL_BLK_NUM+SDMMC_UBOOT_BLK_NUM;
	smdkv210_mmc_partitions[2].sec_num = SDMMC_ROOTFS_128MB_BLK_NUM;
	/* user date  */
	smdkv210_mmc_partitions[3].start_sec = SDMMC_ROOTFS_128MB_BLK_NUM+SDMMC_KERNEL_BLK_NUM+SDMMC_UBOOT_BLK_NUM ;
	smdkv210_mmc_partitions[3].sec_num = card_size_in_blocks-SDMMC_ROOTFS_128MB_BLK_NUM-SDMMC_KERNEL_BLK_NUM-SDMMC_UBOOT_BLK_NUM;
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	add_disk(md->disk);
#if SMDKV210_MMC_SUPPORT_MULTI_PART
	if(0==smdkv210_init_sdmmc_parition(get_capacity(md->disk))){
	   for(i = 0; i < SMDKV210_MMC_MAX_PARTITIONS; i++){
	  	 	printk(KERN_INFO "edu id = %d start_id = %d num = %d\n",i,smdkv210_mmc_partitions[i].start_sec,smdkv210_mmc_partitions[i].sec_num);
			hd = add_partition(md->disk, i, smdkv210_mmc_partitions[i].start_sec, smdkv210_mmc_partitions[i].sec_num, ADDPART_FLAG_NONE);
	   }
	}
#endif
