...
//gionee bsp tangwei 20160112 modify for vib vol 1.8V and freq 235 begin
...
//gionee bsp tangwei 20160112 modify for vib vol 1.8V and freq 235 end

struct pinctrl *pinctrl_drv2604l;
struct pinctrl_state *drv2604l_en_output0, *drv2604l_en_output1;



static int drv2604l_reg_read(struct DRV2604L_data *pDrv2604ldata, unsigned int reg)
{
	int result = i2c_smbus_read_byte_data(pDrv2604ldata->client, reg);
	return result;
}

static int drv2604l_reg_write(struct DRV2604L_data *pDrv2604ldata, unsigned char reg, char val)
{
    int result = i2c_smbus_write_byte_data(pDrv2604ldata->client, reg, val);
	return result;
}
...
static int drv2604l_bulk_read(struct DRV2604L_data *pDrv2604ldata, unsigned char reg, unsigned int count, u8 *buf)
{
	int result = 0, i;
	for(i=0; i < count; i++){
		result = i2c_smbus_read_byte_data(pDrv2604ldata->client, reg+i);
		if(result < 0){
			DRV2604L_DEBUG("%s, read error (%d)\n", __FUNCTION__, result);
			break;
		}
		buf[i] = (u8)result;
	}
...
{
	unsigned char *pbyteBuf = kzalloc(count + 1, GFP_KERNEL);
	int result = 0;
	...
	pbyteBuf[0] = reg;
	memcpy(&pbyteBuf[1], buf, count);

	result = i2c_master_send(pDrv2604ldata->client, pbyteBuf, count + 1);
...
static int drv2604l_set_bits(struct DRV2604L_data *pDrv2604ldata, unsigned char reg, unsigned char mask, unsigned char val)
{
	...
	unsigned char regval = drv2604l_reg_read(pDrv2604ldata, reg);

	temp = regval & ~mask;
	temp |= val & mask;

	if(temp != regval){
		result = drv2604l_reg_write(pDrv2604ldata, reg, temp);
	}
...
static void drv2604l_change_mode(struct DRV2604L_data *pDrv2604ldata, char work_mode, char dev_mode)
{
	...
	DRV2604L_DEBUG("drv2604l_change_mode,mode=%d.\n",dev_mode);
...
static void drv2604l_firmware_load(const struct firmware *fw, void *context)
{
	...
	if(fw != NULL){
		...
		if((pDrv2604ldata->fw_header.fw_magic != DRV2604L_MAGIC)
			...
			DRV2604L_DEBUG("%s, firmware good\n", __FUNCTION__);
...
static int Haptics_init(struct DRV2604L_data *pDrv2604ldata)
{
    ...
    if (reval < 0)
    {
        printk("drv2604: error getting major number %d\n", reval);
        goto fail0;
    }
    ...
    {
        printk("drv2604: error creating class\n");
        goto fail1;
    }
    ...
    {
        printk("drv2604: error creating device 2604\n");
        goto fail2;
    }
    ...
    if (reval)
    {
        printk("drv2604: fail to add cdev\n");
        goto fail3;
    }
    ...
    {
        printk("drv2604: fail to create timed output dev\n");
        goto fail3;
    }
...
static void dev_init_platform_data(struct DRV2604L_data *pDrv2604ldata)
{
	...
	if(pDrv2604ldata->OTP == 0) {
		...
		DRV2604L_DEBUG("%s, OTP programmed\n", __FUNCTION__);
	}
...
static int dev_auto_calibrate(struct DRV2604L_data *pDrv2604ldata)
{
	...
	drv2604l_change_mode(pDrv2604ldata, MODE_DIAGNOSTICS, DEV_READY);
	drv2604l_set_go_bit(pDrv2604ldata, GO);
	...
	DRV2604L_DEBUG("%s, calibration status =0x%x\n", __FUNCTION__, status);
...
static int drv2604l_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
	...
	pDrv2604ldata = kzalloc(sizeof(struct DRV2604L_data),GFP_KERNEL);
	if (pDrv2604ldata == NULL){
		printk(KERN_ERR"%s:no memory\n", __FUNCTION__);
		return -ENOMEM;
	}

	pDrv2604ldata->client = client;
#if 0
	...
	{
		DRV2604L_DEBUG("pDrv2604ldata->PlatData.GpioEnable\n");
		mt_set_gpio_mode(pDrv2604ldata->PlatData.GpioEnable, GPIO_MODE_00);
		mt_set_gpio_dir(pDrv2604ldata->PlatData.GpioEnable, GPIO_DIR_OUT);
		mt_set_gpio_out(pDrv2604ldata->PlatData.GpioEnable, GPIO_OUT_ONE);
    ...
    pinctrl_select_state(pinctrl_drv2604l, drv2604l_en_output1);
	udelay(30);
	...
	//Gionee <gn_by_charging> <lilubao> <20170426> add for change vibrate begin
	printk(KERN_ERR"in %s device_id->%d\n", __FUNCTION__,pDrv2604ldata->device_id);
	//Gionee <gn_by_charging> <lilubao> <20170426> add for change vibrate end
	
	switch (pDrv2604ldata->device_id)
	{
		case DRV2605_VER_1DOT1:
		DRV2604L_DEBUG("drv2604 driver found: drv2605 v1.1.\n");
		break;
		case DRV2605_VER_1DOT0:
		DRV2604L_DEBUG("drv2604 driver found: drv2605 v1.0.\n");
		break;
		case DRV2604:
		DRV2604L_DEBUG("drv2604 driver found: drv2604.\n");
		break;
		case DRV2604L:
		DRV2604L_DEBUG("drv2604 driver found: drv2604L.\n");
		break;
		case DRV2605L:
		DRV2604L_DEBUG("drv2604 driver found: drv2605L.\n");
		break;
		default:
		DRV2604L_DEBUG("drv2604 driver found: unknown.\n");
		break;
	}
    ...
    DRV2604L_DEBUG("drv2604 probe succeeded\n");
...
#if 0//tangwei modify 20160106
	mt_set_gpio_out(pDrv2604ldata->PlatData.GpioEnable, GPIO_OUT_ZERO);
#endif

    pinctrl_select_state(pinctrl_drv2604l, drv2604l_en_output0);
udelay(30);
	if(pDrv2604ldata){
		kfree(pDrv2604ldata);
	}
...
static int drv2604l_remove(struct i2c_client* client)
{
    ...
    DRV2604L_DEBUG(KERN_ALERT"drv2604 remove");

    return 0;
}

static const struct of_device_id drv2604l_of_match[] = {
	{.compatible = "mediatek,DRV2604L"},
	{},
};
...
static int drv2604l_pinctrl_probe(struct platform_device *pdev){
	int retval = 0;

    printk("pi5usb func=%s  line=%d\n", __func__, __LINE__);

    pinctrl_drv2604l = devm_pinctrl_get(&pdev->dev);
    if (IS_ERR(pinctrl_drv2604l)) {
        printk("pi5usb Cannot find usb pinctrl!\n");
    } else {
        drv2604l_en_output0 = pinctrl_lookup_state(pinctrl_drv2604l, "drv2604l_en_low");
        if (IS_ERR(drv2604l_en_output0))
            printk("pi5usb Can *NOT* find pi5usb_en_low\n");
        else
            printk("pi5usb Find pi5usb_en_low\n");

        drv2604l_en_output1 = pinctrl_lookup_state(pinctrl_drv2604l, "drv2604l_en_high");
        if (IS_ERR(drv2604l_en_output1))
...
static int drv2604l_pinctrl_remove(struct platform_device *pdev){
	return 0;
}

static const struct of_device_id drv2604l_pinctrl_of_match[] = {
	{.compatible = "mediatek,drv2604l_pinctrl",},
	{},
};



static struct platform_driver drv2604l_pinctrl_driver = {
	.probe = drv2604l_pinctrl_probe,
	.remove = drv2604l_pinctrl_remove,
	.driver = {
		.name = "drv2604l_pinctrl",
		.owner = THIS_MODULE,
		.of_match_table = drv2604l_pinctrl_of_match,
...
static struct i2c_driver drv2604l_driver =
{
    .driver = {
		...
		.owner = THIS_MODULE,
				.of_match_table = drv2604l_of_match,
    },
...
#if 0
static struct i2c_board_info __initdata i2c_drv2604l_hw  ={
		I2C_BOARD_INFO(HAPTICS_DEVICE_NAME, DRV2604L_I2C_ADDR),
...
static int __init drv2604l_init(void)
{
	...
	int err=0;
	//i2c_register_board_info(DRV2604L_I2C_BUS_ID, &i2c_drv2604l_hw, ARRAY_SIZE(i2c_drv2604l_hw));
	//	i2c_register_board_info(DRV2604L_I2C_BUS_ID, &i2c_drv2604l_hw, 1);
	
		err = platform_driver_register(&drv2604l_pinctrl_driver);
	
	if(err!=0)
	{
	printk("drv2604l_init add pinctrl drviver error\n");
	return err;
	}
	else
	{
	printk("drv2604l_init add pinctrl drviver success\n");
	}	
	

	err=i2c_add_driver(&drv2604l_driver);
	if(err!=0)
	{
	printk("drv2604l_init add i2cdrviver error\n");
	}
	else
	{
	printk("drv2604l_init add i2cdrviver success\n");
	}
	return err;
