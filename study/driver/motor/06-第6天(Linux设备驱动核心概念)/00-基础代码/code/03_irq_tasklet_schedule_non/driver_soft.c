...
软中断的使用步骤：
1、找到NR_SOFTIRQS所在的枚举变量，在NR_SOFTIRQS的前面加上SOFT_TEST，并重新编译烧写内核
2、在驱动程序初始化阶段加上open_softirq(SOFT_TEST,sof_action);
3、在中断服务程序中加上raise_softirq(SOFT_TEST);
4、实现软中断函数 
static void sof_action(struct softirq_action *a)
{
    可以没有任何内容，只是打印一句话 
}
5、包含头文件#include <linux/interrupt.h>，并准备调试，方法：
5.1、如果直接编译，不会报错，但运行时，会提示未知的open_softirq和raise_softirq，原因是没有EXPORT_SYMBOL()导出申请
5.2、所以只能将driver.c加入到内核源码和内核一起编译，只需加入文件到drivers/char/下面，并修改该目录下的Makefile，编译即可通过，运行也没有问题
5.3、如果想编译成模块，那么打开kernel/softirq.c文件加上下面两句，并重新编译烧写内核即可：
...
static int __init demo_module_init(void)
{
	...
	open_softirq(SOFT_TEST,sof_action);
	ret = demo_gph_init();
