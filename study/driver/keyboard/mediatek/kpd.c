...
//gionee add by gongdb start
//#ifdef GN_DUAL_LCM
#define GN_MTK_BSP_HALL_KEY_SUPPORT
//#endif

#ifdef CONFIG_OF
	int hall_irq;
#endif
//gionee add by gongdb end
...
#endif
//GioneeDrv add by gongdb start
#ifdef GN_MTK_BSP_HALL_KEY_SUPPORT
#include <linux/switch.h>
#include <linux/kthread.h>
#include <linux/sched.h>
#define GN_HALL_KEY_OPEN 288
#define GN_HALL_KEY_CLOSE 289
...
static int gn_hall_eint_handler(void *unused );
...
static DRIVER_ATTR(kpd_hall_state,	S_IRUGO,	kpd_show_hall_state,	NULL);
#endif
//GioneeDrv add by gongdb end
...
//extern void switch_mipi_lcm(int sel_lcm);

//extern void gn_turn_on_backlight();
//extern void gn_turn_off_backlight();

//extern void gtp_in_working(int sel);	
//extern void gtp_out_working(int sel);	
extern void aw9523b_switch(int sel);

//extern int get_probe_end_flag();

//extern u8 ucAutoUpgradeAll;
//extern u8 ucAutoUpgradeAll_ex;

//extern bool get_mtkfb_early_suspend();

int get_clam_status(void)
{
	return s_gn_clam;
}
//Gionee add by liush for BFL7506A 20150429 start
EXPORT_SYMBOL(get_clam_status);
//Gionee add by liush for BFL7506A 20150429 end

static void report_hall_key(int sel)
{
	if(!kpd_input_dev) {
	...
	if(!sel) { //hall open
		input_report_key(kpd_input_dev, GN_HALL_KEY_OPEN, 1);
		input_report_key(kpd_input_dev, GN_HALL_KEY_OPEN, 0);
		input_sync(kpd_input_dev);
		switch_set_state((struct switch_dev *)&hall_data, HALL_OPEN);
	}
	else { // Hall close
		input_report_key(kpd_input_dev, GN_HALL_KEY_CLOSE, 1);
		input_report_key(kpd_input_dev, GN_HALL_KEY_CLOSE, 0);
		input_sync(kpd_input_dev);
		switch_set_state((struct switch_dev *)&hall_data, HALL_CLOSE);
	}
}
//liuyuntao 20170221 add lcd switch
//liuyuntao 20170328
extern int gn_mtkfb_lcd_suspend(void);
extern void gn_mtkfb_lcd_resume(int suspended);
extern void gn_lcd_tp_switch(int sel_lcm);
extern void gn_touch_suspend(int sel_tp);
extern void gn_touch_resume(int sel_tp);
extern int gn_lcd_backlight_control(int bl_mode);
extern int get_probe_end_flag(void);//liuyuntao 20170227
extern int tp_fw_upgrade_finished;
extern int in_tp_fw_upgrade_finished;

//Gionee <gn_by_liuyuntao> <liuyuntao> <2017-03-30> add for #76909 begin 
int hal_switch_status = 0;
void gn_clam_act(int sel)
{
	//LCM_DRIVER * lcm_drv=NULL;
	int is_suspend;
	if(!get_probe_end_flag())
		return;
	//liuyuntao switch after fw updated
	if ((in_tp_fw_upgrade_finished == 0) || (tp_fw_upgrade_finished == 0))
		return;
	...
	aw9523b_switch(sel);
//liuyuntao
	printk("liuyuntao gn_clam_act 001\n");

	is_suspend = gn_mtkfb_lcd_suspend();
	gn_touch_suspend(sel);
	if (sel == 0)
	{	
		gn_lcd_backlight_control(1);
	}
	else
	{	
		gn_lcd_backlight_control(3);
	}
	gn_lcd_tp_switch(sel);

    gn_mtkfb_lcd_resume(is_suspend);
	gn_touch_resume(sel);
	if (sel == 0)
	{
		gn_lcd_backlight_control(2);
	}
	else
	{	
		gn_lcd_backlight_control(0);
	}
	
	hal_switch_status = 0;
//Gionee <gn_by_liuyuntao> <liuyuntao> <2017-03-30> add for #76909 end
//liuyuntao

	//gn_turn_off_backlight();
	...
	//if(lcm_drv != NULL){
	//	lcm_drv->switch_mipi(sel);
	//}

	//msleep(200);
	//if (!get_mtkfb_early_suspend())
	//	gn_turn_on_backlight();
}
//liuyuntao 20170221 add lcd switch

#ifdef CONFIG_OF
static int hall_eint_state = 0;
#endif
...
static int gn_hall_eint_handler(void *unused)
{
	...
	do{
		...
		printk(KPD_SAY "%s: kpd_hallkey_state = %d\n",__func__,s_gn_clam);
		
		//gionee SunLiang   modified 2013-09-05 START
		gn_clam_act(s_gn_clam);
		//gionee SunLiang   modified 2013-09-05 END

		// for detecting the return to old_state 
#ifdef CONFIG_OF
		enable_irq(hall_irq);
#else
		mt_eint_set_polarity(CUST_EINT_FLIP_NUM, s_gn_clam);
		mt_eint_unmask(CUST_EINT_FLIP_NUM);
#endif
...
#ifdef CONFIG_OF
static irqreturn_t kpd_hall_eint_handler(int irq, void *data)
{
	printk("%s\n",__func__);
	
	disable_irq_nosync(hall_irq);
	
	if (hall_eint_state == 0) {
...
{
		//mt65xx_eint_mask(GN_EINT_HALL_NUM);
...
static void kpd_keymap_handler(unsigned long data)
{
	...
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad begin
	kpd_info("in [%s] before by lilubao\n",__FUNCTION__);
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad end
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			if (hw_keycode >= KPD_NUM_KEYS)
				continue;
			//Gionee <gn_by_charging> <lilubao> <20170426> add for change keypad begin
			printk("hw_keycode = %d ,i = %d, j = %d \n",hw_keycode,i,j); 
			//Gionee <gn_by_charging> <lilubao> <20170426> add for change keypad end
			...
			//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad begin
			kpd_info("in [%s] linux_keycode->%d\n",__FUNCTION__,linux_keycode);
			//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad end
	...
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad begin
	kpd_info("in [%s] after by lilubao\n",__FUNCTION__);
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad end
}
...
#ifdef GN_MTK_BSP_HALL_KEY_SUPPORT
    case OPEN_HALL_KEY:
		printk("[AUTOTEST] OPEN HALL KEY!!\n");
		input_report_key(kpd_input_dev, GN_HALL_KEY_OPEN, 1);
		input_report_key(kpd_input_dev, GN_HALL_KEY_OPEN, 0);
		input_sync(kpd_input_dev);
		break;
	case CLOSE_HALL_KEY:
		printk("[AUTOTEST] CLOSE HALL KEY!!\n");
		input_report_key(kpd_input_dev, GN_HALL_KEY_CLOSE, 1);
		input_report_key(kpd_input_dev, GN_HALL_KEY_CLOSE, 0);
		input_sync(kpd_input_dev);
		break;
...
#if KPD_HAS_SLIDE_QWERTY
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad begin
	kpd_info("in [%s] define KPD_HAS_SLIDE_QWERTY\n",__FUNCTION__);
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad end
...
#ifdef GN_MTK_BSP_HALL_KEY_SUPPORT
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad begin
	kpd_info("in [%s] define GN_MTK_BSP_HALL_KEY_SUPPORT\n",__FUNCTION__);
	//Gionee <gn_by_charging> <lilubao> <20170421> add for change keypad end
	__set_bit(GN_HALL_KEY_OPEN, kpd_input_dev->keybit);
	__set_bit(GN_HALL_KEY_CLOSE, kpd_input_dev->keybit);
#endif
//GioneeDrv add by gongdb end
#ifdef KPD_KEY_MAP
...
//GioneeDrv add by gongdb start
	...
	{	
			...
			r = switch_dev_register(&hall_data);
			if(r) {
				printk(KPD_SAY "register switch failed (%d)\n", r);
			...
			gn_thread_hall = kthread_run(gn_hall_eint_handler, 0, hall_data.name);
       		if (IS_ERR(gn_thread_hall)) {
   				printk(KPD_SAY "register kthread_run failed (%d)\n", r);
   				//free_irq(MT_KP_IRQ_ID,NULL);
   				misc_deregister(&kpd_dev);
   				input_unregister_device(kpd_input_dev);
   				return r;
...
#ifdef CONFIG_OF
	node = of_find_compatible_node(NULL, NULL, "mediatek, FLIP-eint"); 
	if(node) {
	    ...
	    ret = request_irq(hall_irq, kpd_hall_eint_handler, IRQF_TRIGGER_LOW, "FLIP-eint", NULL);
	    if(ret > 0) 
	...
	//enable_irq(hall_irq);
#else
	mt_eint_registration(CUST_EINT_FLIP_NUM, EINTF_TRIGGER_LOW, kpd_hall_eint_handler, 0);
	mt_eint_unmask(CUST_EINT_FLIP_NUM);  
#endif
