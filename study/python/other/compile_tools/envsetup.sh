...
read_product_config()
{
    if [ -e "${MTK_PROJECTCONFIG_MK}" ]; then
        while read line
        do
        ...
        echo "*** ${MTK_PROJECTCONFIG_MK} NOT Exists ***";
    fi
}
...
if [ -e ${MTK_PROJECTCONFIG_MK} ]; then
    ...
    gn_cdefs=""
    gn_kernel_cdefs=""
    GN_KERNEL_CFG_FILE=`grep 'GN_KERNEL_CFG_FILE' ${MTK_PROJECTCONFIG_MK} | sed 's/ //g' | awk -F= '{print $2}'`
    GN_KERNEL_TMP_FILE=`grep 'GN_KERNEL_TMP_FILE' ${MTK_PROJECTCONFIG_MK} | sed 's/ //g' | awk -F= '{print $2}'`
...
#           echo CONFIG_$LINE >> $GN_KERNEL_CFG_FILE
    ...
    GN_BOOTABLE_TMP_FILE=`grep 'GN_BOOTABLE_TMP_FILE' ${MTK_PROJECTCONFIG_MK} | sed 's/ //g' | awk -F= '{print $2}'`
    export GN_BOOTABLE_TMP_FILE
    if [ `expr length "$GN_BOOTABLE_TMP_FILE"` -gt 0 ];then
        echo GN_CFLAGS := $gn_cdefs > $GN_BOOTABLE_TMP_FILE
    fi

    GN_KCONFIG_TMP_FILE=`grep 'GN_KCONFIG_TMP_FILE' ${MTK_PROJECTCONFIG_MK} | sed 's/ //g' | awk -F= '{print $2}'`
